...
const _getLensCluster = function (cluster) {
  // DEBUG TODO: how to search all existing clusters? look in the source?
...
const _addMetadata = function (cloudUrl, clusterPartials, models) {
  ...
  clusterPartials.forEach((partial, idx) => {
    ...
    model.metadata.labels = {
      namespace: partial.namespace, // DEBUG TODO add custom prefix to this?
    };
...
const _addToCatalog = function (models) {
  ...
  models.forEach((model) => {
    ...
    // DEBUG TODO: need to save the model to a Store so we can restore it on load...
